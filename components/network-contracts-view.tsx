...
import { X, Clock, Trophy, Zap, Target, Shield, Flame, Coins, Radio, ChevronRight } from "lucide-react"
...
export function NetworkContractsView({
  ...
  const [expandedContract, setExpandedContract] = useState<NetworkContractWithClaimed | null>(null)
        ...
        <div className="relative p-4 md:p-6">
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <div className="flex flex-col items-center gap-2 min-w-[70px]">
              <div className={`p-3 rounded-lg border-2 ${difficultyClass}`}>{getTypeIcon(contract.type)}</div>
              <span className={`text-xs font-bold uppercase px-2 py-1 rounded border ${difficultyClass}`}>
                {contract.difficulty}
              </span>
            </div>
            ...
            <div className="flex-1 space-y-2 min-w-0">
              <h3 className="text-lg font-bold text-cyan-400" style={{ fontFamily: "monospace" }}>
                {contract.name}
              </h3>
                ...
                <div className="w-full h-2 bg-black/50 border border-cyan-400/30 rounded-full overflow-hidden">
                  <div
                    className={`h-full bg-gradient-to-r transition-all duration-500 ${
                      canClaim ? "from-green-400 to-cyan-400" : "from-cyan-400/50 to-magenta-400/50"
                    }`}
            ...
            <div className="flex flex-col items-end gap-2 min-w-[130px]">
              ...
              {canClaim ? (
                <Button
                  onClick={(e) => {
                    e.stopPropagation()
                    console.log("[v0] Claim button clicked for contract:", contract.id, contract.name)
                ...
                <div className="w-full text-center text-xs text-cyan-400/50 font-bold py-2">IN PROGRESS</div>
              )}
          ...
          <div className="md:hidden space-y-3">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-2 flex-1 min-w-0">
                <div className={`p-2 rounded-lg border ${difficultyClass} shrink-0`}>{getTypeIcon(contract.type)}</div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-base font-bold text-cyan-400" style={{ fontFamily: "monospace" }}>
                    {contract.name}
                  </h3>
              ...
              <div className={`px-2 py-1 rounded-lg border ${difficultyClass} text-xs font-bold uppercase shrink-0`}>
                {contract.difficulty}
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-cyan-500/30">
              ...
              {canClaim ? (
                <Button
                  onClick={(e) => {
                    e.stopPropagation()
                    console.log("[v0] Claim button clicked (mobile) for contract:", contract.id, contract.name)
                    onClaimReward(contract.id, contract.refreshType)
                  }}
                ...
                <div className="text-center text-xs text-cyan-400/50 font-bold py-2">IN PROGRESS</div>
              )}
            </div>
          </div>
        </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-cyan-300/70">Active Contracts // Protocol Directives</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <Button onClick={onClose} variant="ghost" size="icon" className="hover:bg-red-500/20 active:scale-95">
          ...
          <div className="p-6 space-y-8">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-cyan-500/20 to-transparent border-l-4 border-cyan-500 rounded-r-lg">
                ...
                <div className="flex-1">
                  <h3 className="text-xl md:text-2xl font-bold text-cyan-400" style={{ fontFamily: "monospace" }}>
                    DAILY DIRECTIVES
                  </h3>
                  <p className="text-xs text-cyan-300/70">24-Hour Rotation // High Frequency Operations</p>
                </div>
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2 text-cyan-300/70 text-sm">
                    <Clock className="w-5 h-5" />
                    <span style={{ fontFamily: "monospace" }}>
                      {dailyContracts[0] && formatTimeRemaining(dailyContracts[0].expiresAt)}
                    </span>
                  </div>
                  <div className="text-right text-cyan-300/70 text-sm" style={{ fontFamily: "monospace" }}>
                    {dailyContracts.filter((c) => c.claimed).length} / {dailyContracts.length} COMPLETE
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{dailyContracts.map(renderContract)}</div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-magenta-500/20 to-transparent border-l-4 border-magenta-500 rounded-r-lg">
                ...
                <div className="flex-1">
                  <h3 className="text-xl md:text-2xl font-bold text-magenta-400" style={{ fontFamily: "monospace" }}>
                    WEEKLY OPERATIONS
                  </h3>
                  <p className="text-xs text-magenta-300/70">7-Day Cycle // Strategic Campaigns</p>
                </div>
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2 text-magenta-300/70 text-sm">
                    <Clock className="w-5 h-5" />
                    <span style={{ fontFamily: "monospace" }}>
                      {weeklyContracts[0] && formatTimeRemaining(weeklyContracts[0].expiresAt)}
                    </span>
                  </div>
                  <div className="text-right text-magenta-300/70 text-sm" style={{ fontFamily: "monospace" }}>
                    {weeklyContracts.filter((c) => c.claimed).length} / {weeklyContracts.length} COMPLETE
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{weeklyContracts.map(renderContract)}</div>
            </div>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-start gap-4 flex-1">
                <div className={`p-4 rounded-lg border-2 ${getDifficultyColor(expandedContract.difficulty)}`}>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-cyan-300/70">
                      {expandedContract.refreshType === "daily" ? "Daily Directive" : "Weekly Operation"}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-2 p-4 bg-black/40 rounded-lg border border-cyan-500/30">
              <h3 className="text-sm font-bold text-cyan-400 uppercase tracking-wider">Objective Brief</h3>
              <p className="text-base text-cyan-300 leading-relaxed">{expandedContract.description}</p>
            </div>

            {/* Progress */}
            <div className="space-y-3 p-4 bg-black/40 rounded-lg border border-cyan-500/30">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-cyan-400 uppercase tracking-wider">Progress Status</h3>
                <span className="text-cyan-400 font-bold text-lg" style={{ fontFamily: "monospace" }}>
              ...
              <div className="w-full h-4 bg-black/50 border border-cyan-400/30 rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${(expandedContract.progress / expandedContract.maxProgress) * 100}%` }}
                />
              </div>
              {expandedContract.progress >= expandedContract.maxProgress && (
            ...
            <div className="flex items-center justify-between p-4 bg-gradient-to-r from-yellow-500/20 to-transparent rounded-lg border-l-4 border-yellow-500">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-yellow-300/70 uppercase">Contract Reward</p>
              ...
              {expandedContract.progress >= expandedContract.maxProgress ? (
                <Button
                  onClick={() => {
                    ...
                    onClaimReward(expandedContract.id, expandedContract.refreshType)
                ...
                <div className="text-cyan-400/50 font-bold text-lg">IN PROGRESS</div>
              )}
            </div>
          </div>
        </div>
