...
export function ProgrammingPanel({
  ...
  const currentMovementPairs =
    movementPairs.length > 0
      ? movementPairs
      : pairs.filter((pair) => {
          const actionDef = AVAILABLE_ACTIONS.find(
            (def) => def.id === pair.action.id,
          );
          return actionDef?.coreType === "movement";
        });
  ...
  const removeMovementProtocol = (index: number) => {
    setEditingMovementProtocols(
      editingMovementProtocols.filter((_, i) => i !== index),
    );
    ...
    const movementPairIndex = pairs.findIndex((p, i) => {
      ...
      return (
        actionDef?.coreType === "movement" &&
        currentMovementPairs.indexOf(p) === index
      );
    });
  ...
  const removeTacticalProtocol = (index: number) => {
    setEditingTacticalProtocols(
      editingTacticalProtocols.filter((_, i) => i !== index),
    );
    // Also remove from actual pairs
    const tacticalPairIndex = pairs.findIndex((p, i) => {
      const actionDef = AVAILABLE_ACTIONS.find((def) => def.id === p.action.id);
      return (
        actionDef?.coreType === "tactical" &&
        currentTacticalPairs.indexOf(p) === index
      );
    });
  ...
  const moveMovementProtocol = (index: number, direction: "up" | "down") => {
    if (
      ...
      (direction === "down" && index === editingMovementProtocols.length - 1)
    )
      return;
  ...
  const moveTacticalProtocol = (index: number, direction: "up" | "down") => {
    if (
      (direction === "up" && index === 0) ||
      (direction === "down" && index === editingTacticalProtocols.length - 1)
    )
      return;
  ...
  const handleSave = () => {
    ...
    const hasIncompleteMovement = editingMovementProtocols.some(
      (p) => !p.triggerId || !p.actionId,
    );
    const hasIncompleteTactical = editingTacticalProtocols.some(
      (p) => !p.triggerId || !p.actionId,
    );
  ...
  const sortedTriggers = [...safeTriggers].sort((a, b) =>
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs sm:text-sm text-cyan-300/70 mt-1">
                    ...
                    Movement • {editingTacticalProtocols.length}/
                    {MAX_TACTICAL_PROTOCOLS} Tactical
                  </p>
                </div>
                <Button
                  variant="ghost"
                ...
                {editingMovementProtocols.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {editingMovementProtocols.map((protocol, index) => {
                      const trigger = safeTriggers.find(
                        (t) => t.id === protocol.triggerId,
                      );
                      const action = safeActions.find(
                          ...
                          <div className="flex items-center gap-4">
                            <div className="flex flex-col gap-1">
                              ...
                              <Button
                                ...
                                disabled={
                                  index === editingMovementProtocols.length - 1
                                }
                              >
                            ...
                            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div>
                                ...
                                <Select
                                  ...
                                  onValueChange={(value) =>
                                    updateMovementProtocolTrigger(index, value)
                                  }
                                >
                                  <SelectTrigger
                          ...
                          <div className="flex items-center gap-4">
                            <div className="flex flex-col gap-1">
                              ...
                              <Button
                                ...
                                disabled={
                                  index === editingTacticalProtocols.length - 1
                                }
                              >
                                <MoveDown className="w-3 h-3" />
                              </Button>
                            ...
                            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div>
                                ...
                                <Select
                                  value={protocol.triggerId}
                                  onValueChange={(value) =>
                                    updateTacticalProtocolTrigger(index, value)
                                  }
                                >
                                  <SelectTrigger
