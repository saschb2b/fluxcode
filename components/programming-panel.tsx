  ...
  return (
    <TooltipProvider delayDuration={300}>
      <div className="fixed inset-0 bg-black/95 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
        <Card className="w-full max-w-6xl h-[90vh] flex flex-col bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 border-2 border-amber-600/40 shadow-2xl shadow-amber-900/30">
          <div className="flex items-center justify-between px-8 py-4 border-b border-amber-600/30 bg-gradient-to-r from-slate-900/80 to-transparent">
            <div className="flex items-center gap-6">
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center text-2xl font-bold border-2 border-amber-500/50">
                P1
              </div>
              <div>
                <h2 className="text-2xl font-bold text-amber-400 tracking-wide">Battle Protocols</h2>
                <p className="text-xs text-slate-400 mt-1">Configure your fighter's AI behavior</p>
              </div>
            </div>
            <Button
              variant="ghost"
          ...
          <div className="flex-1 overflow-hidden flex">
            <div className="w-80 border-r border-amber-600/30 bg-slate-900/30 p-6 flex flex-col gap-4">
              <div className="text-sm font-bold text-amber-400 uppercase tracking-wider mb-2">Add New Protocol</div>

              <div className="space-y-4">
                <div>
                  <label className="text-xs text-slate-400 uppercase tracking-wider mb-2 block">Condition</label>
                  <Select
                  ...
                  >
                    <SelectTrigger className="bg-slate-950/80 border-slate-700 text-slate-200 h-10">
                    ...
                    <SelectContent className="bg-slate-950 border-slate-700 max-h-60">
                      {unlockedTriggers.map((trigger) => (
                        <SelectItem
                          key={trigger.id}
                          value={trigger.id}
                          className="text-slate-200 focus:bg-cyan-500/20 focus:text-cyan-300 py-3"
                        >
                          <div className="flex flex-col gap-1">
                            <div className="font-medium">{trigger.name}</div>
                            <div className="text-xs text-slate-400 leading-relaxed">{trigger.description}</div>
                          </div>
                        </SelectItem>
                      ))}
                    </SelectContent>
                  ...
                  {selectedTrigger && (
                    <p className="text-xs text-slate-500 mt-2 leading-relaxed">{selectedTrigger.description}</p>
                  )}
                </div>

                <div>
                  <label className="text-xs text-slate-400 uppercase tracking-wider mb-2 block">Action</label>
                  <Select
                    ...
                    <SelectContent className="bg-slate-950 border-slate-700 max-h-60">
                      {unlockedActions.map((action) => (
                        <SelectItem
                          key={action.id}
                          value={action.id}
                          className="text-slate-200 focus:bg-rose-500/20 focus:text-rose-300 py-3"
                        >
                          <div className="flex flex-col gap-1">
                ...
                <Button
                  className="w-full h-10 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-500 hover:to-amber-600 text-white font-bold tracking-wide shadow-lg disabled:opacity-50"
                ...
                <div className="space-y-2 pr-4">
                  {sortedPairsWithIndices.length === 0 ? (
                    <div className="text-center text-slate-600 py-16">
                      ...
                      <p className="text-sm">No protocols configured</p>
                      <p className="text-xs mt-2">Add your first protocol to begin</p>
                    </div>
                  ) : (
                    sortedPairsWithIndices.map(({ pair, originalIndex }, displayIndex) => {
                      ...
                      return (
                        <div
                          key={originalIndex}
                          className={`group relative flex items-center gap-3 h-14 rounded-lg border ${borderColor} bg-gradient-to-r ${barColor} backdrop-blur-sm transition-all duration-200 hover:scale-[1.02] ${!isEnabled ? "opacity-40" : ""}`}
                        >
                          <div className="flex items-center justify-center w-16 h-full border-r border-slate-700/50">
                            <button
                            ...
                            >
                              <div
                                className={`absolute top-0.5 ${isEnabled ? "right-0.5" : "left-0.5"} w-5 h-5 rounded-full bg-white transition-all shadow-lg`}
                              />
                              <span
                                className={`absolute inset-0 flex items-center justify-center text-[10px] font-bold ${isEnabled ? "text-white" : "text-slate-500"}`}
                              >
                                {isEnabled ? "ON" : "OFF"}
                              </span>
                            </button>
                          ...
                          <div className="flex items-center justify-center w-10 h-10 rounded-full bg-slate-900/80 border border-amber-500/50 text-amber-400 font-bold text-sm">
                            {displayIndex + 1}
                          </div>
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <div
                                className={`flex-1 flex items-center gap-4 ${textColor} font-medium text-sm cursor-help`}
                              >
                                <span className="min-w-[200px]">{pair.trigger.name}</span>
                                <span className="text-amber-400">→</span>
                                <span className="min-w-[150px]">{pair.action.name}</span>
          ...
          <div className="px-8 py-4 border-t border-amber-600/30 bg-gradient-to-r from-slate-900/80 to-transparent flex justify-end">
            <Button
              onClick={onClose}
              size="lg"
              className="px-8 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-500 hover:to-amber-600 text-white font-bold tracking-wide shadow-lg"
            >
              Done
            </Button>
          </div>
