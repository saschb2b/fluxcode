...
export class BattleEngine {
  ...
  constructor(
    ...
    console.log(
      "[v0] BattleEngine constructor - playerMovementPairs count:",
      playerMovementPairs.length,
    );
    console.log(
      "[v0] BattleEngine constructor - playerTacticalPairs count:",
      playerTacticalPairs.length,
    );
    console.log(
      "[v0] BattleEngine constructor - playerMovementPairs:",
      playerMovementPairs.map(
        (p) => `${p.trigger.id}->${p.action.id} (priority: ${p.priority})`,
      ),
    );
    console.log(
      "[v0] BattleEngine constructor - playerTacticalPairs:",
      playerTacticalPairs.map(
        (p) => `${p.trigger.id}->${p.action.id} (priority: ${p.priority})`,
      ),
    );
    console.log(
        ...
        : [
            {
              ...
              corrosiveStacks: ensureArray(
                (initialState as any).enemyCorrosiveStacks,
              ),
    ...
    this.state = {
      ...
      enemyCorrosiveStacks:
        ensureArray((initialState as any).enemyCorrosiveStacks) || [],
      shieldRegenDisabled: initialState.shieldRegenDisabled || false,
    ...
    if (initialState.enemies && initialState.enemies.length > 0) {
      ...
      if (hasEmbeddedPairs) {
        ...
        this.enemyPairs = initialState.enemies.map((e: any) => {
          ...
          console.log(
            `[v0] BattleEngine constructor - Enemy ${e.id} has ${pairs.length} protocols from enemy object`,
          );
        ...
        if (
          ...
          "trigger" in firstElement &&
          "action" in firstElement
        ) {
          this.enemyPairs = [
    ...
    console.log(
      "[v0] BattleEngine constructor - enemy count:",
      this.state.enemies.length,
    );
    console.log(
      "[v0] BattleEngine constructor - enemyPairs arrays:",
      this.enemyPairs.length,
    );
  ...
  tick(deltaTime: number): BattleUpdate {
    ...
    this.state.enemies.forEach((enemy) => {
      enemy.burnStacks = enemy.burnStacks.filter(
        (stack) => stack.endTime > this.battleTime,
      );
      enemy.viralStacks = enemy.viralStacks.filter(
        (stack) => stack.endTime > this.battleTime,
      );
      enemy.empStacks = enemy.empStacks.filter(
        (stack) => stack.endTime > this.battleTime,
      );
      enemy.lagStacks = enemy.lagStacks.filter(
        (stack) => stack.endTime > this.battleTime,
      );
    ...
    update.enemyHP = this.state.enemies.reduce((sum, e) => sum + e.hp, 0);
    update.enemyShields = this.state.enemies.reduce(
      (sum, e) => sum + e.shields,
      0,
    );
    update.enemyArmor = this.state.enemies.reduce((sum, e) => sum + e.armor, 0);
    update.burnStacks = this.state.enemies.reduce(
      (sum, e) => sum + e.burnStacks.length,
      0,
    );
    update.viralStacks = this.state.enemies.reduce(
      (sum, e) => sum + e.viralStacks.length,
      0,
    );
    update.empStacks = this.state.enemies.reduce(
      (sum, e) => sum + e.empStacks.length,
      0,
    );
    update.lagStacks = this.state.enemies.reduce(
      (sum, e) => sum + e.lagStacks.length,
      0,
    );
    update.displaceStacks = this.state.enemies.reduce(
      (sum, e) => sum + e.displaceStacks.length,
      0,
    );
    update.corrosiveStacks = this.state.enemies.reduce(
      (sum, e) => sum + e.corrosiveStacks.length,
      0,
    );
    ...
    const guardiansDefeated =
      guardiansInBattle.length > 0 && guardiansAlive.length === 0;
    ...
    this.state.enemies.forEach((enemy, i) => {
      console.log(
        `[v0] Enemy ${i} (${enemy.isPawn ? "pawn" : "guardian"}): HP=${enemy.hp}/${enemy.maxHp}`,
      );
    });
    ...
    if (guardiansDefeated || allEnemiesDefeated) {
      ...
      update.battleHistory = this.battleHistory;
      console.log(
        `[v0] Victory! ${guardiansDefeated ? "Guardian(s) defeated" : "All enemies defeated"}`,
      );
      return update;
    }

    const playerMovementAction = this.executeMovementCore(
    ...
    if (!playerMovementAction) {
      const playerTacticalAction = this.executeTacticalCore(
    ...
    this.state.enemies.forEach((enemy, index) => {
      if (enemy.hp > 0) {
        ...
        const enemyMovementAction = this.executeEnemyMovementCore(
          enemyPairs,
          enemy,
          deltaTime,
        );
        if (enemyMovementAction) {
          const actionUpdate = this.applyAction(
            enemyMovementAction,
            false,
            enemy,
          );
        ...
        if (!enemyMovementAction) {
          const enemyTacticalAction = this.executeEnemyTacticalCore(
            enemyPairs,
            enemy,
            deltaTime,
          );
  ...
  private executeMovementCore(
    pairs: TriggerActionPair[],
    isPlayer: boolean,
    deltaTime: number,
  ): ActionResult | null {
    const movementPairs = pairs.filter(
      (pair) => pair.action.coreType === "movement",
    );
  ...
  private executeTacticalCore(
    pairs: TriggerActionPair[],
    isPlayer: boolean,
    deltaTime: number,
  ): ActionResult | null {
    const tacticalPairs = pairs.filter(
      (pair) => pair.action.coreType === "tactical",
    );
  ...
  private executeEnemyMovementCore(
    ...
    if (!enemy.position) {
      console.error(
        "[v0] Enemy position is undefined, skipping Movement Core execution",
      );
      return null;
    }
    ...
    for (const pair of movementPairs) {
      ...
      if (enemy.lagStacks.length > 0) {
        const failureChance = enemy.lagStacks.reduce(
          (acc, stack) => acc + stack.actionFailureChance,
          0,
        );
      ...
      if (triggerResult) {
        console.log(
          `[v0] Enemy ${enemy.id} Movement Core executing: ${pair.trigger.id} -> ${pair.action.id}`,
        );
        const adjustedCooldown = pair.action.cooldown * lagCooldownMultiplier;
        this.movementCooldowns.set(cooldownKey, adjustedCooldown);
        return {
  ...
  private executeEnemyTacticalCore(
    ...
    if (!enemy.position) {
      console.error(
        "[v0] Enemy position is undefined, skipping Tactical Core execution",
      );
      return null;
    }
    ...
    for (const pair of tacticalPairs) {
      ...
      if (enemy.lagStacks.length > 0) {
        ...
        if (Math.random() < failureChance) {
          console.log(
            `[v0] Enemy ${enemy.id} tactical action stuttered due to Lag`,
          );
          continue;
        }
      }
      ...
      if (triggerResult) {
        console.log(
          `[v0] Enemy ${enemy.id} Tactical Core executing: ${pair.trigger.id} -> ${pair.action.id}`,
        );
        const adjustedCooldown = pair.action.cooldown * lagCooldownMultiplier;
        this.tacticalCooldowns.set(cooldownKey, adjustedCooldown);
        return {
    ...
    for (const pair of pairs) {
      ...
      if (this.actionCooldowns.has(cooldownKey)) {
        ...
        if (!isPlayer && remainingCooldown > 100) {
          ...
          console.log(
            `[v0] Enemy action ${pair.action.id} on cooldown: ${Math.round(remainingCooldown)}ms remaining`,
          );
        }
        continue;
      }
      ...
      if (!isPlayer && this.state.enemyLagStacks.length > 0) {
        const failureChance = this.state.enemyLagStacks.reduce(
          (acc, stack) => acc + stack.actionFailureChance,
          0,
        );
        if (Math.random() < failureChance) {
          console.log(
            `[v0] Enemy action stuttered due to Lag (${Math.round(failureChance * 100)}% chance)`,
          );
      ...
      if (triggerResult) {
        console.log(
          `[v0] ${isPlayer ? "Player" : "Enemy"} executing: ${pair.trigger.id} -> ${pair.action.id}`,
        );
        const adjustedCooldown = pair.action.cooldown * lagCooldownMultiplier;
        if (
          !isPlayer &&
          lagCooldownMultiplier > 1.0 &&
          pair.action.cooldown > 0
        ) {
          console.log(
  ...
  private executeEnemyAI(
    ...
    console.log(
      `[v0] Enemy ${enemy.id} executing AI with ${pairs.length} protocols`,
    );
    console.log(
    ...
    for (const pair of pairs) {
      ...
      if (this.actionCooldowns.has(cooldownKey)) {
        console.log(
          `[v0] Enemy ${enemy.id} skipping ${pair.trigger.id}->${pair.action.id} (on cooldown)`,
        );
        continue;
      }
      ...
      if (enemy.lagStacks.length > 0) {
        ...
        if (Math.random() < failureChance) {
          console.log(
            `[v0] Enemy ${enemy.id} action stuttered due to Lag (${Math.round(failureChance * 100)}% chance)`,
          );
          continue;
        }
      }
      ...
      const triggerResult = pair.trigger.check(context);
      console.log(
        `[v0] Enemy ${enemy.id} trigger ${pair.trigger.id} result: ${triggerResult}`,
      );

      if (triggerResult) {
        console.log(
          `[v0] Enemy ${enemy.id} executing: ${pair.trigger.id} -> ${pair.action.id}`,
        );
        const adjustedCooldown = pair.action.cooldown * lagCooldownMultiplier;
        this.actionCooldowns.set(cooldownKey, adjustedCooldown);
        return {
      ...
      case "drain":
        ...
        if (isPlayer) {
          const newHP = Math.min(
            this.state.playerHP + (action.heal || 10),
            100,
          );
        ...
        if (action.position) {
          if (!isPlayer && enemy) {
            if (action.position.x < 3) {
              console.log(
                `[v0] Enemy ${enemy.id} blocked from entering player territory at x=${action.position.x}`,
              );
              return update;
            }
      ...
      case "dash-attack":
        if (action.position) {
          if (!isPlayer && enemy) {
            if (action.position.x < 3) {
              console.log(
                `[v0] Enemy ${enemy.id} blocked from dash-attacking into player territory`,
              );
            ...
            if (isOccupied) {
              console.log(
                `[v0] Enemy ${enemy.id} blocked from dash-attacking to occupied tile`,
              );
              const dashProj = this.createProjectile(
                isPlayer,
                action.damage || 15,
      ...
      case "retreat-shot":
        if (action.position) {
          if (!isPlayer && enemy) {
            if (action.position.x < 3) {
              console.log(
                `[v0] Enemy ${enemy.id} blocked from retreating into player territory`,
              );
              // Still fire the projectile, but don't move
            ...
            if (isOccupied) {
              console.log(
                `[v0] Enemy ${enemy.id} blocked from retreat-shot to occupied tile`,
              );
              const retreatProj = this.createProjectile(
                isPlayer,
                action.damage || 12,
      ...
      case "heal-over-time":
        ...
        for (let i = 0; i < hotTicks; i++) {
          setTimeout(() => {
            if (isPlayer) {
              const newHP = Math.min(
                this.state.playerHP + (action.healPerTick || 3),
                100,
              );
              this.state.playerHP = newHP;
            } else {
              const newHP = Math.min(
                enemy!.hp + (action.healPerTick || 3),
                enemy!.maxHp,
              );
              enemy!.hp = newHP;
            }
  ...
  private applyDamageToPlayer(proj: Projectile): {
    ...
    if (remainingDamage > 0) {
      ...
      console.log(
        `[v0] Player took ${Math.round(remainingDamage)} HP damage (${this.state.playerHP} HP remaining)`,
      );
    }
  ...
  private applyDamageToEnemy(
    ...
    if (
      ...
      const MAX_CORROSIVE_STACKS = 999999; // Allow infinite
      const CORROSIVE_DURATION = 999999; // Permanent during battle

      if (
        enemy.corrosiveStacks.length < MAX_CORROSIVE_STACKS &&
        enemy.armor > 0
      ) {
  ...
  private getViralDamageMultiplier(): number {
    ...
    const stackCount = this.state.enemies.reduce(
      (sum, e) => sum + e.viralStacks.length,
      0,
    );
